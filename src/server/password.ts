 ...
 *
 * For backward compatibility, also supports verifying argon2 hashes
 * when the argon2 package is available (Node/Docker path).
 */
 ...
 * Supports both PBKDF2 format and legacy argon2 format.
 */
export async function checkPassword(
  ...
  // Legacy argon2 path — try to load argon2 (only available in Node)
  try {
    const argon2 = await import("argon2");
    const valid = await argon2.verify(storedPassword, providedPassword);
    if (!valid) throw new Error("Invalid password");
  } catch (err) {
    // If argon2 isn't available or fails, throw a clear error
    if (err instanceof Error && err.message === "Invalid password") {
      throw err;
    }
    throw new Error(
      "Legacy password format requires argon2 which is not available in this environment",
    );
  }
}
